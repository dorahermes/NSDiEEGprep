...
f = makeimagestack_wrapper(stimuliOrd(:,:,ii_sort(1:20),:));
imshow(uint8(f))

subplot(2,3,5)
[bb_sort,ii_sort] = sort(mean(thisBB(tt>.100 & tt<.200,:)),'descend');
f = makeimagestack_wrapper(stimuliOrd(:,:,ii_sort(1:20),:));
imshow(uint8(f))

subplot(2,3,6)
[bb_sort,ii_sort] = sort(mean(thisBB(tt>.200 & tt<.300,:)),'descend');
f = makeimagestack_wrapper(stimuliOrd(:,:,ii_sort(1:20),:));
imshow(uint8(f))
